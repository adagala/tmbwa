    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Payment confirmations appear only after reconciliation is complete.
        </p>
      </div>
      {error ? (
        <p
      ...
      {role === 'administrator' ? (
        <section className="space-y-3">
          <h2 className="font-semibold">Delivery operations</h2>
          {deliveries.map((delivery) => (
            <Card
              key={delivery.id}
              className="grid items-center gap-2 text-sm sm:grid-cols-5"
            >
              <span className="break-all font-medium">{delivery.eventId}</span>
              <span>Member: {delivery.memberId}</span>
          ...
          {!deliveries.length ? (
            <p className="text-sm text-gray-500">No delivery jobs yet.</p>
          ) : null}
        </section>
      ) : null}
    </div>
