...
export const addPayment = ({ contribution, payment }: { contribution: Contribution; payment: Payment }) =>
  call('recordContributionPayment', {
    ...
    paymentDateMillis: payment.paymentdate instanceof Date
...
export const deletePayment = ({ contribution, payment }: { contribution: Contribution; payment: Payment }) =>
  call('reverseContributionPayment', {
    memberId: contribution.member_id,
    paymentId: payment.payment_id,
  });

export const addContribution = ({ month, member }: { uid: string; month: string; member: Member }) =>
  call('createContribution', { memberId: member.member_id, month });

export const deleteContribution = ({ contribution, member }: { contribution: Contribution; member: Member }) =>
  call('removeContribution', {
    memberId: member.member_id,
    contributionId: contribution.month,
  });

export const updateMemberBalance = ({ member, balanceForm }: { uid: string; member: Member; balanceForm: MemberBalanceForm }) =>
