...
export const statementRows = (
  ...
  [
    ...contributions.map((item) => ({
      date: item.month,
      description: `Monthly contribution – ${item.month.slice(0, 7)}`,
      charge: item.amount,
      payment: 0,
      reference: item.contribution_id,
    })),
    ...payments.map((item) => {
      const isDeduction =
...
export const printStatement = (
  ...
  const rows = statementRows(contributions, payments).filter(
  ...
  main.append(
    textElement(
      popup.document,
      'p',
      `Period: ${from || 'Beginning'} – ${to || 'Current'}`,
    ),
  );
  main.append(
    textElement(
      popup.document,
      'p',
      `Account balance: ${kenyaMoney.format(member.balance)} | Outstanding contributions: ${kenyaMoney.format(outstanding)}`,
    ),
  );
  const table = popup.document.createElement('table');
  table.style.cssText = 'width:100%;border-collapse:collapse';
  const header = table.insertRow();
  ['Date', 'Description', 'Reference', 'Charge', 'Payment'].forEach((label) =>
    header.append(textElement(popup.document, 'th', label)),
  );
  rows.forEach((row) => {
    ...
    [
      ...
      row.description,
      row.reference,
      kenyaMoney.format(row.charge),
      kenyaMoney.format(row.payment),
    ].forEach((value) => line.append(textElement(popup.document, 'td', value)));
  });
  main.append(
    table,
    textElement(
      popup.document,
      'p',
      `Current account balance: ${kenyaMoney.format(member.balance)}`,
    ),
  );
  popup.print();
};
